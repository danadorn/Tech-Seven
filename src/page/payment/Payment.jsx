    ...
    <div className={`min-h-screen p-4 mt-15 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-800'}`}>
      <div className={`max-w-2xl mx-auto p-10 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className={`flex justify-between mb-6 pb-2 border-b ${darkMode ? 'border-gray-600' : 'border-gray-200'}`}>
          ...
          <div className={`px-2 py-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>2. Delivery</div>
          <div className={`px-2 py-1 ${darkMode ? 'text-blue-400 font-bold' : 'text-blue-600 font-bold'}`}>3. Payment</div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-6">
            ...
            <div className="space-y-4">
              <div className={`p-4 border rounded-lg ${darkMode ? 'border-gray-600 bg-gray-700' : 'border-gray-300 bg-white'}`}>
                <div className="mb-4">
                  <label className={`block mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Card number</label>
                  <input
                    type="text"
                    name="number"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={`block mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>MM/YY</label>
                    <input
                      type="text"
                      name="expiry"
                  ...
                  <div>
                    <label className={`block mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>CVV</label>
                    <input
                      type="text"
                      name="cvv"
              ...
              <div className="flex items-center">
                ...
                />
                <label htmlFor="sameAddress">Billing Address same as contact</label>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-3">
              <div 
                className={`p-3 border rounded-lg cursor-pointer ${paymentMethod === 'aba' ? (darkMode ? 'border-blue-400 bg-blue-900/20' : 'border-blue-500 bg-blue-50') : (darkMode ? 'border-gray-600 hover:border-gray-500' : 'border-gray-300 hover:border-gray-400')}`}
                onClick={() => setPaymentMethod('aba')}
              >
                <p className="font-medium">ABA</p>
              </div>
              <div 
                className={`p-3 border rounded-lg cursor-pointer ${paymentMethod === 'paypal' ? (darkMode ? 'border-blue-400 bg-blue-900/20' : 'border-blue-500 bg-blue-50') : (darkMode ? 'border-gray-600 hover:border-gray-500' : 'border-gray-300 hover:border-gray-400')}`}
          ...
          <div className="mb-6 flex items-center">
            <input
              ...
              className={`mr-2 h-5 w-5 rounded ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'}`}
            />
            <label htmlFor="saveInfo" className="font-medium">Save my information and create account</label>
          </div>

          <div className={`p-4 rounded-lg mb-6 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            <div className="flex justify-between items-center">
              <span className="font-medium">Total</span>
              <span className="text-xl font-bold">$7,607.14</span>
            </div>
          </div>
